...
int main(void) {
    ...
    printf("  T    KeyGen    RandPK    ShareSign_1    ShareSign_2    ShareSign_3    Combine    Verify\n");
    printf("-------------------------------------------------------------------------------------------\n");

    for (int t_idx = 0; t_idx < num_tests; t_idx++) {
        int T = thresholds[t_idx];
        int N = T; /* Full consensus: all N parties must sign */

        /* Heap allocate to avoid stack overflow at T=1024 */
        ...
        double time_keygen, time_randpk = 0,
               time_sign1 = 0, time_sign2 = 0, time_sign3 = 0,
               time_combine = 0, time_verify = 0;
        ...
        /* ======== 2. RANDPK ========
         *
         * Measure a single derive_session_pk call, which is representative of
         * the per-session cost.  We reuse party 0's pairwise seed with itself
         * (a deterministic stand-in for seed_{i,j}) and the session ID derived
         * from the message.
         *
         * This corresponds directly to Algorithm 3 in the paper.
         */
        clock_gettime(CLOCK_MONOTONIC, &t_start);
        {
            /* Derive session ID the same way wallet_sign_round1 does */
            uint8_t sid[SESSION_ID_BYTES];
            HSessionID(sid, msg, msglen, ctr);

            /* Build a public_key_t view of mpk for derive_session_pk */
            public_key_t pk_view;
            pk_view.A = mpk.A;
            pk_view.t = mpk.t;

            session_pk_t spk;
            /* Use pairwise seed of party 0 with itself as the per-session seed */
            derive_session_pk(&spk, &pk_view,
                              parties[0].pairwise_seeds[0], sid);
        }
        clock_gettime(CLOCK_MONOTONIC, &t_end);
        time_randpk = diff_ms(t_start, t_end);

        /* ======== 3. SHARESIGN_1 (Round 1 – Commitments) ======== */
        clock_gettime(CLOCK_MONOTONIC, &t_start);
        for (int i = 0; i < T; i++) {
        ...
        /* ======== 4. SHARESIGN_2 (Broker: aggregate commitments + challenge) ======== */
        clock_gettime(CLOCK_MONOTONIC, &t_start);
        {
            polyvec_k_t w_agg;
            ...
            /* ======== 5. SHARESIGN_3 (Round 3 – Responses) ======== */
            clock_gettime(CLOCK_MONOTONIC, &t_end);
            ...
            if (!z_shares || !m_rows) {
                printf("Allocation failed in combine for T=%d\n", T);
                return -1;
            }
            for (int i = 0; i < T; i++) {
            ...
            printf("%4d  %8.3f  %8.3f  %13.3f  %13.3f  %13.3f  %9.3f  %9.3f\n",
                   T, time_keygen, time_randpk,
                   time_sign1, time_sign2, time_sign3,
                   time_combine, time_verify);
        }
